...
const all_products = {};
for (const handle of ['144-146-trifecta-heavyweight-tee','144-146-the-birds-tee']) {
  all_products[handle] = await (await fetch('https://zenkaiclothing.com/products/'+handle+'.js')).json();
}
...
engine.options.globals = {all_products, routes:context.routes, shop:context.shop, cart:context.cart};
