...
const out=path.join(root,'output/impostors-30-journeys');
...
engine.registerFilter('file_url',file=>'https://zenkaiclothing.com/cdn/shop/files/'+file);
...
const templates=(await fs.readdir(path.join(root,'templates'))).filter(f=>/^product\.imp-journey-/.test(f));
const reports=[];
for(const filename of templates){
  ...
  const isNine=type==='impostors-ad-journey';
