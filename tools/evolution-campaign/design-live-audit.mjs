...
for(const width of [390,1440]){
 ...
 for(const route of routes){
  ...
  check(response.status()===200,'HTTP '+response.status());check(metrics.documentWidth<=width,'horizontal overflow');check(!metrics.brokenImages.length,'broken images');check(!metrics.badCopy,'unwanted copy');check(metrics.form?.id===route.variant&&metrics.form?.quantity==='1','native product form mismatch');check(metrics.ctaHeight>=44,'CTA too small');check(metrics.duplicateThumbnails===0,'old thumbnail layout');
