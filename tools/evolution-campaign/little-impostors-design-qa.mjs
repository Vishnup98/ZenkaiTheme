 ...
 for(const [width,height] of sizes){
  ...
  for(const selector of ['.ec-included','.mc-details-grid','.mc-community','.ec-faq']){
   ...
   for(let frame=0;frame<3;frame++){
    ...
    const state=await page.evaluate(()=>{const sticky=document.querySelector('[data-ec-sticky]'),s=sticky.getBoundingClientRect(),button=sticky.querySelector('button'),b=button.getBoundingClientRect(),inline=[...document.querySelectorAll('[data-ec-main-cta],[data-ec-inline-cta]')].filter(e=>!e.closest('[data-ec-sticky]')).some(e=>{const r=e.getBoundingClientRect();return r.top>=0&&r.bottom<=innerHeight});return {inline,hidden:sticky.hidden,fits:s.left>=0&&s.right<=innerWidth&&s.bottom<=innerHeight+1&&b.left>=s.left&&b.right<=s.right&&b.height>=44,buttonLines:b.height};});
