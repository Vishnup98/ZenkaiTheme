...
for(const width of [320,390,768,1440]){
 ...
 for(const route of routes){
  ...
  if(width<900){check(!metrics.separateControlRow,'separate mobile navigation row');if(route.family!=='evolution-creative')check(!metrics.mobileArrowsVisible,'mobile arrows visible');check(metrics.slideWidth<metrics.railWidth-25,'next card not visible');check(metrics.railHeight<=300,'oversized mobile rail');}
