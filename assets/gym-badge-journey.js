(function () {
  ...
  if (regionCarousel) {
    ...
    // A selected region can be the right-hand card in the desktop viewport.
    // Keep that explicit selection while a programmatic scroll settles.
    var requestedRegionIndex = null;
    ...
    function updateRegionControls() {
      ...
      activeRegionIndex = requestedRegionIndex === null ? nearestIndex : requestedRegionIndex;

      regionTabs.forEach(function (tab, index) {
        tab.setAttribute('aria-current', index === activeRegionIndex ? 'true' : 'false');
      });
      if (regionPrev) regionPrev.disabled = regionViewport.scrollLeft <= 1;
      if (regionNext) regionNext.disabled = regionViewport.scrollLeft + regionViewport.clientWidth >= regionViewport.scrollWidth - 1;
    }
    ...
    function showRegion(index) {
      ...
      requestedRegionIndex = nextIndex;
    ...
    if (regionViewport) {
      ...
      function releaseRegionTarget() { requestedRegionIndex = null; }
      regionViewport.addEventListener('pointerdown', releaseRegionTarget, { passive: true });
      regionViewport.addEventListener('touchstart', releaseRegionTarget, { passive: true });
      regionViewport.addEventListener('wheel', releaseRegionTarget, { passive: true });
